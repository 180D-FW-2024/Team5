...
class MazeWindow(QMainWindow):
    def __init__(self, n, m):
        ...
        self.server_host = '100.94.211.35' # Replace with RPi's IP address
    ...
    def receive_camera_data(self):
        try:
            ...
            while True:
                ...
                print(f"Expected frame length: {frame_len}")
                ...
                while len(frame_data) < frame_len:
                    packet = camera_socket.recv(min(4096, frame_len - len(frame_data)))
                    if not packet:
                        print("Connection closed before full frame received.")
                        return
                    frame_data += packet
                
                print(f"Received frame length: {len(frame_data)}")
