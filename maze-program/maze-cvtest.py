...
class MazeWindow(QMainWindow):
    def __init__(self, n, m):
        ...
        self.setGeometry(100, 50, 1450, 900)  #x, y, L, H
    ...
    def setup_socket_camera(self):
        """Set up the socket for camera stream"""
        try:
            camera_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            # camera_socket.settimeout(5.0)  # Timeout after 5 seconds
            camera_socket.connect((self.server_host, self.camera_port))
            print(f"Connected to camera at {self.server_host}:{self.camera_port}")
            return camera_socket
        except socket.timeout:
            print("Camera connection timed out.")
            return None
        except Exception as e:
    ...
    def receive_camera_data(self):
        ...
        try:
            while True: 
                ...
                frame_size_data = self.socket_camera.recv(4)
                ...
                while len(frame_data) < frame_size:
                    frame_data += self.socket_camera.recv(4096)
